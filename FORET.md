...
- [x] **Current-drawdown MDD + Portfolio hero parity:** MDD across the app now answers "how far below the all-time high am I right now?" instead of "what was my worst YTD peak-to-trough decline?" — `calculateCurrentDrawdown()` takes the full snapshot history plus the live value, finds the ATH, and measures the current decline from it, so recovering to a new high resets the stat to 0.00% instead of preserving an old scar. MDD (1D) keeps its consecutive day-over-day definition but now scans the entire history ("largest single-day drop ever recorded"), not just the current year. Both stats moved into one shared `useDrawdownStats()` hook in `usePortfolio.ts` feeding the Dashboard Net Worth strip and the Portfolio hero, which grew from 4 to 6 desktop columns (YTD Start, MDD, MDD (1D), Exposure, Positions, YTD P&L). Lesson: drawdown percentages are scale-invariant, so the same unscaled snapshot series serves both the investor-filtered net worth view and the full portfolio view — one hook, no per-page math.
