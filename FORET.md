...
- [x] **SPX default benchmark + Yahoo history hardening:** the Portfolio % vs Benchmarks chart now defaults to BTC, ETH, and SPX. SPX uses Yahoo `^GSPC`, and the Yahoo provider fetches historical benchmark data through `yahooFinance.chart()` before falling back to the raw chart endpoint. Lesson: Yahoo search/quote had already taught us production IPs need the library's crumb/cookie path; benchmark history needed that same treatment.
