...
- [x] **Asian equity search + local price hints:**
  ...
  - Review follow-up: fallback JPY/TWD/KRW rates are allowed for display hints only. Creating a non-USD listed-equity position, editing one, or adding to one now waits for a real FX rate before persisting cost basis, so fallback estimates cannot permanently skew average cost or P&L.
