...
Root lockfile is owned by **npm 10.8.2** (declared in `package.json`, matching Node 20 CI) — regenerate with that version and verify a clean `npm ci`; npm 11 rewrites optional-peer metadata npm 10 rejects (`@emnapi/*`).

Root `package.json` overrides `exceljs`'s transitive `uuid` to `11.1.1` (`GHSA-w5hq-g745-h8pq`; ExcelJS only uses `v4()`). Never `npm audit fix --force` — it downgrades ExcelJS to 3.4.0. Keep `npm audit` clean. ExcelJS treats worksheet name `History` as protected; exports use `Snapshots` (`export.test.ts`).

### Coolify Deployment Verification

`.github/workflows/deploy-backend.yml` must verify the exact deployment Coolify returns — during a rolling rebuild the old container stays healthy for minutes, so sleep + `/health` probing is a false positive. Parse `deployment_uuid` from `POST /api/v1/deploy`, poll it to a successful terminal state, verify its commit when returned, then health-check. `COOLIFY_API_TOKEN` needs `read` + `deploy` permissions.
...
See **Dev Demo Route** (mocked `/api`, `/dev/demo`) and **Local QA Auth Bypass** (sanitized real-API scale QA); never with production data/builds.
...
- Vercel env: `VITE_API_URL` must be the full `/api/v1` path (verify `curl .../api/v1/health/db`); `VITE_WS_BACKEND_URL` required for prod WebSocket (warns + disables if missing).
